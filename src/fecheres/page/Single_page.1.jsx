...
export default function Single_page() {
    ...
    console.log(url);
    return (
        <div className='flex h-[100vh] bg-gray-800 text-white  gap-[280px]'>{data.map((item, index) => {
            return;
        })}
            {/* <div className=' p-1 w-[50px] md:w-1/5 bg-[#3456] h-[100vh] sticky flex flex-col md:items-end gap-4 pt-4   top-0 ' >
             ...
             <div className='flex justify-center items-center '>
               ...
               <div className='w-1/2'>
                    <img className='w-full h-[200px]' src={data?.image} alt="image" />
                </div>
                <div className='h-[100%] flex flex-col   justify-between '>
                    <p>Name:{data?.full_name}</p>
                    <p>Price:{data?.birthdate}</p>
                    <p>Code:{data?.country}</p>
